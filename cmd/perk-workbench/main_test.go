...
func TestOfficialPluginEntries_orderPathAndTrust(t *testing.T) {
	restore := installOfficialManifestFixture(t)
	defer restore()

	entries, trust, err := officialPluginEntries(nil)
	if err != nil {
		t.Fatalf("officialPluginEntries = %v", err)
	}
	if len(entries) != 4 {
	...
	for i, name := range []string{"sqlite", "mysql", "postgres", "mongodb"} {
		...
		if trust[entries[i]] == "" {
			t.Fatalf("trust has no digest for %q", entries[i])
		}
	}
}

func TestOfficialPluginEntries_disabledSidecarIsNotResolved(t *testing.T) {
	restore := installOfficialManifestFixture(t)
	defer restore()

	entries, trust, err := officialPluginEntries([]string{"postgres"})
	if err != nil {
		t.Fatalf("officialPluginEntries = %v", err)
	}
	if len(entries) != 3 || len(trust) != 3 {
		t.Fatalf("entries/trust = %d/%d, want 3/3", len(entries), len(trust))
	}
	for _, entry := range entries {
		if strings.Contains(entry, "perk-postgres") {
			t.Fatalf("disabled postgres sidecar resolved: %v", entries)
		}
	}
}
...
func TestOfficialPluginEntries_failsClosedForDigestAndPendingMetadata(t *testing.T) {
	...
	mysqlPath := filepath.Join(filepath.Dir(host), "plugins", "perk-mysql")
	if err := os.WriteFile(mysqlPath, []byte("changed"), 0o755); err != nil {
		t.Fatal(err)
	}
	if _, _, err := officialPluginEntries(nil); err == nil || !strings.Contains(err.Error(), "executable sha256 mismatch") {
		t.Fatalf("digest error = %v, want executable sha256 mismatch", err)
	}

	manifest := officialManifest{SchemaVersion: 1, Protocol: plugin.ProtocolVersion, TestedHostCommit: strings.Repeat("a", 40)}
	manifest.Plugins = []officialManifestPlugin{{Name: "sqlite", Protocol: plugin.ProtocolVersion, TestedRef: manifest.TestedHostCommit, Targets: map[string]officialManifestAsset{
		runtime.GOOS + "/" + runtime.GOARCH: {Status: "pending"},
	}}}
	...
	if _, _, err := officialPluginEntries([]string{"mysql", "postgres", "mongodb"}); err == nil || !strings.Contains(err.Error(), "incomplete release metadata") {
		t.Fatalf("pending metadata error = %v, want incomplete release metadata", err)
	}
}

func TestOfficialPluginEntries_failsClosedForMissingSidecar(t *testing.T) {
	restore := installOfficialManifestFixture(t)
	defer restore()

	host := testOfficialHostPath(t)
	if err := os.Remove(filepath.Join(filepath.Dir(host), "plugins", "perk-sqlite")); err != nil {
		t.Fatal(err)
	}
	if _, _, err := officialPluginEntries(nil); err == nil || !strings.Contains(err.Error(), "perk-sqlite") {
		t.Fatalf("missing sidecar error = %v, want perk-sqlite", err)
	}
}

func testOfficialHostPath(t *testing.T) string {
	t.Helper()
	path, err := officialExecutable()
	if err != nil {
		t.Fatal(err)
	}
	return path
...
func installOfficialManifestFixture(t *testing.T) func() {
	t.Helper()
	previousExecutable := officialExecutable
	previousManifest := officialManifestData

	hostDir := t.TempDir()
	pluginDir := filepath.Join(hostDir, "plugins")
	...
	names := []string{"sqlite", "mysql", "postgres", "mongodb"}
	digests := make(map[string]string, len(names))
	for _, name := range names {
		path := filepath.Join(pluginDir, "perk-"+name)
		...
		digests[name], err = plugin.SHA256File(path)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	manifest := officialManifest{SchemaVersion: 1, Protocol: plugin.ProtocolVersion, TestedHostCommit: commit}
	for _, name := range names {
		assetURL := "https://example.invalid/" + name
		assetSHA := strings.Repeat("b", 64)
		executableSHA := digests[name]
		manifest.Plugins = append(manifest.Plugins, officialManifestPlugin{
			Name: name, Protocol: plugin.ProtocolVersion, TestedRef: commit,
			Targets: map[string]officialManifestAsset{
				runtime.GOOS + "/" + runtime.GOARCH: {
					Status: "verified", AssetURL: &assetURL, AssetSHA256: &assetSHA,
					Target:     officialManifestTarget{GOOS: runtime.GOOS, GOARCH: runtime.GOARCH},
					Executable: "perk-" + name, ExecutableSHA256: &executableSHA,
				},
			},
		})
	}
	...
	officialExecutable = func() (string, error) {
		return filepath.Join(hostDir, "perk-workbench"), nil
	}
	officialManifestData = data
	return func() {
		officialExecutable = previousExecutable
		officialManifestData = previousManifest
	}
}

func TestOfficialPluginLoadFailureIsFatal(t *testing.T) {
	restore := installOfficialManifestFixture(t)
	defer restore()
	t.Setenv("PERK_PLUGIN_HELPER", "1")
	...
	for _, name := range []string{"sqlite", "mysql", "postgres", "mongodb"} {
		path := filepath.Join(filepath.Dir(testOfficialHostPath(t)), "plugins", "perk-"+name)
	...
	officialManifestData = data
	loader, err := loadOfficialAndConfiguredPlugins(context.Background(), app.Config{}, func(database.Shim) error {
		return errors.New("registered twice")
	})
	if loader != nil {
		t.Cleanup(func() { _ = loader.Close() })
	}
	if err == nil || !strings.Contains(err.Error(), "official plugin startup failed") || !strings.Contains(err.Error(), "registered twice") {
		t.Fatalf("loadOfficialAndConfiguredPlugins error = %v, want fatal registration failure", err)
	}
}
