...
func TestKeepRows_capsAtMaxRows(t *testing.T) {
	docs := make([]bson.D, sharedsql.MaxRows+10)
	for i := range docs {
		docs[i] = bson.D{{Key: "n", Value: i}}
	}
	kept, truncated := keepRows(docs)
	if len(kept) != sharedsql.MaxRows || !truncated {
		t.Fatalf("keepRows(%d) = %d rows, truncated=%t; want %d with truncation", len(docs), len(kept), truncated, sharedsql.MaxRows)
	}
	small := docs[:5]
...
func TestBrowseFilter(t *testing.T) {
	query := browseFilter([]sharedsql.BrowseFilter{
		{Column: "borough", Operator: sharedsql.BrowseFilterEqual, Value: "Bronx"},
