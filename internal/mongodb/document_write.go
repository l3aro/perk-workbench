...
func (s *Service) WriteCapabilities() sharedsql.WriteCapabilities {
	return sharedsql.WriteCapabilities{
...
// document.
func (s *Service) ReadDocument(ctx context.Context, collection string, id sharedsql.DocumentPayload) (sharedsql.DocumentPayload, error) {
	...
	return sharedsql.DocumentPayload{Format: sharedsql.DocumentFormatMongoExtendedJSON, Data: data}, nil
}
...
// mutating data.
func (s *Service) InsertDocument(ctx context.Context, collection string, doc sharedsql.DocumentPayload) (sharedsql.Result, error) {
	if doc.Format != sharedsql.DocumentFormatMongoExtendedJSON {
		return sharedsql.Result{}, fmt.Errorf("unsupported document format %s", doc.Format)
	}
...
func (s *Service) ReplaceDocument(ctx context.Context, collection string, id sharedsql.DocumentPayload, doc sharedsql.DocumentPayload) (sharedsql.Result, error) {
	identity, err := parseIdentity(id)
	if err != nil {
		return sharedsql.Result{}, err
	}
	...
	if existing, found := docValue(replacement, "_id"); found {
		if !bsonEqual(existing, identity) {
			return sharedsql.Result{}, errors.New("replacement _id does not match the requested identity")
		}
	} else {
	...
	return sharedsql.Result{RowsAffected: result.MatchedCount}, nil
}

// DeleteDocument removes the document identified by id.
func (s *Service) DeleteDocument(ctx context.Context, collection string, id sharedsql.DocumentPayload) (sharedsql.Result, error) {
	identity, err := parseIdentity(id)
	if err != nil {
		return sharedsql.Result{}, err
	}
	...
	return sharedsql.Result{RowsAffected: result.DeletedCount}, nil
}
...
func decodeDocument(payload sharedsql.DocumentPayload) (bson.D, error) {
...
func parseIdentity(payload sharedsql.DocumentPayload) (any, error) {
	if payload.Format != sharedsql.DocumentFormatMongoExtendedJSON {
		return nil, fmt.Errorf("unsupported document format %s", payload.Format)
	}
