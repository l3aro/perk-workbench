...
func Open(ctx context.Context, target string) (*Service, error) {
	...
	service.info = driver.DatabaseInfo{Product: "MongoDB", Version: version}
	return service, nil
}
...
func (s *Service) ListSchema(ctx context.Context) ([]driver.SchemaObject, error) {
	...
	sort.Strings(names)
	objects := make([]driver.SchemaObject, 0, len(names)+1)
	objects = append(objects, driver.SchemaObject{Database: s.db.Name(), Type: "database", Name: s.db.Name()})
	for _, name := range names {
		objects = append(objects, driver.SchemaObject{Database: s.db.Name(), Type: "collection", Name: name})
	}
	return objects, nil
}
...
func (s *Service) execute(ctx context.Context, input string, readOnly bool) (driver.Result, error) {
	statement, err := parseStatement(input)
	if err != nil {
		return driver.Result{}, err
	}
	if readOnly && writeMethods[statement.method] {
		return driver.Result{}, fmt.Errorf("read-only connection: %s is not allowed", statement.method)
	}
	started := time.Now()
...
func (s *Service) executeCommand(ctx context.Context, st statement, started time.Time) (driver.Result, error) {
	...
	return driver.Result{}, fmt.Errorf("unsupported method %q", st.method)
}
...
func keepRows(docs []bson.D) ([]bson.D, bool) {
	...
	return docs[:maxRows], true
}

func countResult(count int64, duration time.Duration) driver.Result {
...
func browseFilter(filters []driver.BrowseFilter) bson.M {
	if len(filters) == 0 {
		return bson.M{}
	}
...
func (s *Service) TableInfo(ctx context.Context, name string) ([]driver.ColumnInfo, error) {
	...
	sort.Strings(order)
	columns := make([]driver.ColumnInfo, 0, len(order))
	for _, name := range order {
		column := driver.ColumnInfo{Name: name, Type: types[name], Nullable: true}
		if name == "_id" {
...
func (s *Service) ListIndexes(ctx context.Context, table string) ([]driver.IndexInfo, error) {
	...
	indexes := make([]driver.IndexInfo, 0, len(specifications))
	for _, specification := range specifications {
		index := driver.IndexInfo{Name: specification.Name}
