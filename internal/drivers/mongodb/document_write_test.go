...
func TestDocumentsResult_emitsDocumentIDs(t *testing.T) {
	...
	for index, payload := range result.DocumentIDs {
		if payload.Format != sharedsql.DocumentFormatMongoExtendedJSON {
...
func TestDocumentWrite_capabilities(t *testing.T) {
	...
	if capabilities.Document == nil || !capabilities.Document.Text || capabilities.Document.Format != sharedsql.DocumentFormatMongoExtendedJSON {
...
func TestIntegration_documentWriteRoundTrip(t *testing.T) {
	...
	inserted, err := service.InsertDocument(ctx, collection, sharedsql.DocumentPayload{Format: format, Data: []byte(document)})
	if err != nil {
	...
	browse, err := service.BrowseTable(ctx, collection, sharedsql.BrowseOptions{Limit: 10})
	if err != nil {
	...
	replaced, err := service.ReplaceDocument(ctx, collection, identity, sharedsql.DocumentPayload{Format: format, Data: []byte(updated)})
	if err != nil {
	...
	replaced, err = service.ReplaceDocument(ctx, collection, identity, sharedsql.DocumentPayload{Format: format, Data: []byte(withID)})
	if err != nil {
	...
	if _, err := service.ReplaceDocument(ctx, collection, identity, sharedsql.DocumentPayload{Format: format, Data: []byte(mismatched)}); err == nil || !strings.Contains(err.Error(), "does not match") {
...
func TestDocumentWrite_rejectsBadPayloads(t *testing.T) {
	...
	other := sharedsql.DocumentPayload{Format: "application/x-other", Data: []byte("{}")}
	valid := sharedsql.DocumentPayload{Format: format, Data: []byte(`{"name": "x"}`)}
	scalarID := sharedsql.DocumentPayload{Format: format, Data: []byte(`{"$oid": "000000000000000000000001"}`)}
	...
	if _, err := service.InsertDocument(ctx, collection, sharedsql.DocumentPayload{Format: format, Data: []byte(`{"name": `)}); err == nil {
		t.Fatal("malformed insert succeeded")
	}
	// Non-document bodies (arrays, scalars) fail.
	if _, err := service.InsertDocument(ctx, collection, sharedsql.DocumentPayload{Format: format, Data: []byte(`[1, 2]`)}); err == nil {
		t.Fatal("array insert succeeded")
	}
	if _, err := service.InsertDocument(ctx, collection, sharedsql.DocumentPayload{Format: format, Data: []byte(`42`)}); err == nil {
...
func TestDocumentWrite_rejectsNonScalarIdentity(t *testing.T) {
	...
	doc := sharedsql.DocumentPayload{Format: format, Data: []byte(`{"a": 1}`)}
	...
	arr := sharedsql.DocumentPayload{Format: format, Data: []byte(`[1]`)}
