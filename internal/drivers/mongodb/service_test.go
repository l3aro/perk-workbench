...
func TestIntegration_documentsAndSchema(t *testing.T) {
	...
	if len(distinct.Rows) != maxRows || !distinct.Truncated {
		t.Fatalf("distinct bulk rows = %d truncated = %t, want %d with truncation", len(distinct.Rows), distinct.Truncated, maxRows)
	}
	...
	browse, err := service.BrowseTable(ctx, "perk_test", driver.BrowseOptions{
		Sorts: []driver.BrowseSort{{Column: "score", Descending: true}},
		Limit: 1,
	})
	if err != nil {
	...
	if err := service.CreateIndex(ctx, "perk_test", driver.IndexChange{Name: "name_idx", Columns: []string{"name"}, Unique: true}); err != nil {
	...
	if err := service.AlterColumn(ctx, "perk_test", driver.ColumnChange{}); err == nil {
