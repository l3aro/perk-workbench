...
func Open(ctx context.Context, dsn string) (*Service, error) {
	...
	return &Service{db: db, info: driver.DatabaseInfo{Product: "MySQL", Version: version}}, nil
}
	...
	for rows.Next() {
		...
		if tableName.Valid {
			...
			object := driver.SchemaObject{
				Database: database,
				Type:     objectType,
				Name:     sanitizeDisplay(tableName.String),
			}
...
func (s *Service) Execute(ctx context.Context, statement string) (result driver.Result, err error) {
	...
	if err != nil {
		return driver.Result{}, fmt.Errorf("acquiring mysql connection: %w", err)
	}
	defer func() {
...
func (s *Service) ExecuteReadOnly(ctx context.Context, statement string) (result driver.Result, err error) {
	...
	if err != nil {
		return driver.Result{}, fmt.Errorf("beginning read-only transaction: %w", err)
	}
	defer func() {
...
func (s *Service) TableInfo(ctx context.Context, name string) ([]driver.ColumnInfo, error) {
	...
	for rows.Next() {
		...
		column.Attributes = sanitizeDisplay(column.Attributes)
		...
		case "UNI":
			column.Indexes = []driver.IndexKind{driver.IndexUnique}
		case "MUL":
			column.Indexes = []driver.IndexKind{driver.IndexRegular}
		}
...
func mysqlColumnDeclaration(change driver.ColumnChange, attributes mysqlColumnAttributes) string {
...
func (s *Service) BrowseTable(ctx context.Context, name string, options driver.BrowseOptions) (driver.Result, error) {
	if options.Offset < 0 || options.Limit < 1 || options.Limit > maxRows {
		return driver.Result{}, fmt.Errorf("invalid browse range: offset=%d limit=%d", options.Offset, options.Limit)
	}
	...
	if len(options.Filters) > 0 {
		...
		for _, filter := range options.Filters {
			if !valid[filter.Column] {
				return driver.Result{}, fmt.Errorf("invalid browse filter column: %s", filter.Column)
			}
			...
			case browseFilterPattern, browseFilterNotPattern:
				...
				// MySQL escapes backslashes inside string literals, so the
				// ESCAPE clause needs a doubled backslash in the SQL text.
			...
			case browseFilterIsNull, browseFilterIsNotNull:
				terms = append(terms, column+" "+string(filter.Operator))
			default:
				return driver.Result{}, fmt.Errorf("invalid browse filter operator: %q", filter.Operator)
			}
		}
