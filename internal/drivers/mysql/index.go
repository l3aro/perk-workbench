...
func (s *Service) ListIndexes(ctx context.Context, table string) ([]driver.IndexInfo, error) {
	...
	for rows.Next() {
		...
		if len(indexes) == 0 || indexes[len(indexes)-1].Name != name {
			indexes = append(indexes, driver.IndexInfo{Name: sanitizeDisplay(name), Unique: nonUnique == 0 && name != "PRIMARY", PrimaryKey: name == "PRIMARY"})
		}
		last := len(indexes) - 1
		indexes[last].Columns = append(indexes[last].Columns, sanitizeDisplay(column))
	}
...
func (s *Service) ListIndexesAll(ctx context.Context) (map[string][]driver.IndexInfo, error) {
	...
	for rows.Next() {
		...
		if info == nil || qualified != lastTable || name != lastName {
			...
			info = &driver.IndexInfo{Name: sanitizeDisplay(name), Unique: nonUnique == 0 && name != "PRIMARY", PrimaryKey: name == "PRIMARY"}
		}
...
func mysqlCreateIndexStatement(table string, change driver.IndexChange) string {
	prefix := "CREATE INDEX "
	if change.Unique {
