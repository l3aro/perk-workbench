...
func TestSchemaLevel_rightOnLeafNoop(t *testing.T) {
	...
	model.schema.list.Select(1) // events
	updated, _ := model.Update(tea.KeyPressMsg{Code: tea.KeyRight})
	model = updated.(Model)
	if model.schema.anim != nil {
...
func TestSchemaLevel_postgresNavigation(t *testing.T) {
	...
	updated, _ := model.Update(tea.KeyPressMsg{Code: 'l'})
	model = updated.(Model)
	if model.schema.anim != nil || model.schema.list.Index() != 1 {
		t.Fatalf("l on main = anim %v, cursor %d, want the public schema at 1", model.schema.anim != nil, model.schema.list.Index())
	}

	// l on the expanded public schema drops to its first table.
	updated, _ = model.Update(tea.KeyPressMsg{Code: tea.KeyRight})
	model = updated.(Model)
	if model.schema.anim != nil || model.schema.list.Index() != 2 {
		t.Fatalf("l on public = anim %v, cursor %d, want accounts at 2", model.schema.anim != nil, model.schema.list.Index())
	}

	// h on a table jumps to its schema row, not the database root.
	model.schema.list.Select(3) // orders
	updated, _ = model.Update(tea.KeyPressMsg{Code: 'h'})
	model = updated.(Model)
	if model.schema.list.Index() != 1 {
		t.Fatalf("h on orders = cursor %d, want the public schema at 1", model.schema.list.Index())
	}

	// h on a collapsed schema jumps to the database root.
	model.schema.list.Select(4) // audit
	updated, _ = model.Update(tea.KeyPressMsg{Code: tea.KeyLeft})
	model = updated.(Model)
	if model.schema.list.Index() != 0 {
		t.Fatalf("h on audit = cursor %d, want the main root at 0", model.schema.list.Index())
	}

	// l on the collapsed audit schema expands it with the accordion.
	model.schema.list.Select(4)
	updated, _ = model.Update(tea.KeyPressMsg{Code: 'l'})
	model = updated.(Model)
	if model.schema.anim == nil || model.schema.anim.collapsing {
